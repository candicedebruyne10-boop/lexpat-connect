...
/**
 * Normalise un intitulé avant comparaison.
 *
 * Indispensable : shortageJobs2026.js écrit « Maître d’hôtel » avec une
 * apostrophe typographique (U+2019), ce fichier « Maître d'hôtel » avec une
 * apostrophe droite (U+0027). Sans normalisation, la comparaison échoue et le
 * simulateur répond « pas d'accès au permis unique » pour un métier qui figure
 * pourtant bien sur la liste VDAB. Même problème sur
 * « Kinderbegeleider baby’s en peuters ».
 *
 * Normalise aussi les espaces multiples, les espaces insécables et la casse.
 */
...
/** Set for O(1) lookup — clés normalisées. */
export const FLANDRE_VDAB_227_SET = new Set(FLANDRE_VDAB_227.map(normalizeJobLabel));

/** Set MB normalisé, à utiliser pour toute comparaison. */
const FLANDRE_MB_21_NORMALIZED = new Set([...FLANDRE_MB_21].map(normalizeJobLabel));

/** Le métier figure-t-il sur la liste de l'arrêté ministériel ? */
export function isMediumSkilledFunction(job) {
  return FLANDRE_MB_21_NORMALIZED.has(normalizeJobLabel(job));
}

/** Le métier figure-t-il sur la liste VDAB des professions en pénurie ? */
export function isVdabShortageJob(job) {
  return FLANDRE_VDAB_227_SET.has(normalizeJobLabel(job));
}

/**
 * Sépare les groupes flamands en deux listes distinctes pour l'affichage public.
 *
 * Règle d'affichage retenue : un métier figurant sur la liste MB apparaît
 * UNIQUEMENT dans la liste 1. Il est retiré de la liste 2, même s'il figure
 * aussi sur la liste VDAB — un employeur doit voir le régime le plus favorable
 * qui s'applique à lui, une seule fois, sans avoir à arbitrer entre deux
 * mentions du même métier.
 *
 * Les groupes vidés de tous leurs métiers sont supprimés du résultat.
 *
 * @param {Array<{title: string, jobs: string[]}>} groups - Groupes de la région flamande
 * @returns {{mbGroups: Array, vdabGroups: Array, mbCount: number, vdabCount: number}}
 */
export function splitFlandreGroups(groups) {
  const mbGroups = [];
  const vdabGroups = [];
  let mbCount = 0;
  let vdabCount = 0;
  ...
  for (const group of groups) {
    const mbJobs = group.jobs.filter((job) => isMediumSkilledFunction(job));
    const vdabJobs = group.jobs.filter((job) => !isMediumSkilledFunction(job));

    if (mbJobs.length) {
      mbGroups.push({ title: group.title, jobs: mbJobs });
      mbCount += mbJobs.length;
    }
    if (vdabJobs.length) {
      vdabGroups.push({ title: group.title, jobs: vdabJobs });
      vdabCount += vdabJobs.length;
    }
  }

  return { mbGroups, vdabGroups, mbCount, vdabCount };
}
