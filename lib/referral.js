...
export function isMissingReferralColumnError(error) {
  const message = error?.message || '';
  return (
    message.includes("referral_code") &&
    (message.includes("worker_profiles") || message.includes("schema cache") || message.includes("column"))
  );
}
...
export async function resolveReferralCode(supabase, code) {
  ...
  if (isMissingReferralColumnError(error)) return null;
