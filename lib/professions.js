...
import { isMediumSkilledFunction } from "./flandreKnelpuntberoepen";
...
/**
 * Libellés des deux régimes flamands, tels qu'ils apparaissent dans le menu
 * déroulant du simulateur. Reprendre exactement ceux de la page publique.
 */
const FLANDRE_REGIME_LABELS = {
  fr: {
    mediumSkilled: "✦ Fonctions moyennement qualifiées — dispense du test marché",
    shortage: "● Professions en pénurie — publication 9 semaines VDAB + EURES"
  },
  en: {
    mediumSkilled: "✦ Medium-skilled functions — labour market test exemption",
    shortage: "● Shortage occupations — 9-week VDAB + EURES publication"
  }
};

/**
 * Couleur du titre de groupe dans le menu déroulant.
 *
 * Safari ignore la couleur sur <optgroup> : les symboles ✦ et ● en préfixe
 * garantissent que la distinction reste lisible même sans couleur.
 */
const FLANDRE_REGIME_TONES = {
  mediumSkilled: "mediumSkilled",
  shortage: "shortage"
};

/**
 * Groupes de métiers proposés dans le simulateur, par région sélectionnée.
 *
 * Cas particulier de la Flandre : les métiers ne sont pas classés par secteur
 * mais par RÉGIME juridique, parce que c'est cette distinction — et non le
 * secteur — qui détermine les démarches de l'employeur. Un employeur voit donc
 * immédiatement, au moment de choisir le métier, s'il devra publier son offre
 * pendant 9 semaines ou non.
 *
 * Les autres régions gardent le classement sectoriel : elles n'ont qu'une seule
 * liste, sans distinction de régime.
 */
export function getProfessionGroupsForRegions(value, locale = "fr") {
  ...
  selections.forEach((regionLabel) => {
    const isFlandre = String(regionLabel).toLowerCase().includes("flandre");

    if (isFlandre) {
      const labels = FLANDRE_REGIME_LABELS[lang];
      const flatJobs = professionOptionsByRegion[regionLabel] || [];

      flatJobs.forEach((job) => {
        if (job === "Autre profession") return;
        const key = isMediumSkilledFunction(job)
          ? labels.mediumSkilled
          : labels.shortage;
        const current = groupsMap.get(key) || new Set();
        current.add(job);
    ...
    regionGroups.forEach((group) => {
      const label = translateGroupTitle(group.label, locale);
      const current = groupsMap.get(label) || new Set();
      group.options.forEach((option) => current.add(option));
      groupsMap.set(label, current);
    });
  });

  // « Fonctions moyennement qualifiées » en tête : c'est le régime le plus
  // favorable, celui que l'employeur a intérêt à repérer en premier.
  const order = [
    FLANDRE_REGIME_LABELS[lang].mediumSkilled,
    FLANDRE_REGIME_LABELS[lang].shortage
  ];
  ...
  return Array.from(groupsMap.entries())
    ...
    .map(([label, options]) => ({
      ...
      tone:
        label === order[0]
          ? FLANDRE_REGIME_TONES.mediumSkilled
          : label === order[1]
            ? FLANDRE_REGIME_TONES.shortage
            : undefined,
