...
import { getEffectiveWorkerProfileVisibility } from "./worker-profile-visibility";
...
export async function notifyWorkersForNewOffer({ supabase, offerId }) {
  ...
  const eligibleWorkers = workers.filter((worker) => getEffectiveWorkerProfileVisibility(worker) === "visible");
  if (!eligibleWorkers.length) return;

  const workerMap = new Map(eligibleWorkers.map((worker) => [worker.id, worker]));
...
export async function notifyEmployersForNewWorker({ supabase, workerProfileId }) {
  ...
  for (const member of members) {
    ...
    const visibleWorkers = (workers || []).filter((row) => getEffectiveWorkerProfileVisibility(row) === "visible");
    const workerMap = new Map(visibleWorkers.map((row) => [row.id, row]));
