...
function buildEmployerDigestRows(rows, locale) {
  return rows.map(({ match, worker }) => ({
    title: worker.target_job || (locale === "en" ? "New talent" : "Nouveau talent"),
...
export async function notifyEmployersForNewWorker({ supabase, workerProfileId }) {
  ...
  for (const member of members) {
    ...
    const locale = resolveLocale(userMetadata.preferred_locale);
    const notificationType = "employer_new_talents";
    ...
    const subject =
      locale === "en"
        ? dedupedWorkers.length > 1
          ? `${dedupedWorkers.length} new talents match your search on LEXPAT Connect`
          : "A new talent matches your search on LEXPAT Connect"
        : dedupedWorkers.length > 1
          ? `${dedupedWorkers.length} nouveaux talents correspondent à votre recherche sur LEXPAT Connect`
          : "Un nouveau talent correspond à votre recherche sur LEXPAT Connect";
