...
// LinkedIn API version — format YYYYMM uniquement (ex: "202501").
// NE PAS utiliser le format avec le jour (ex: "20250401" est invalide).
// LinkedIn supporte les versions dans une fenêtre glissante de 12-24 mois.
// Si LINKEDIN_API_VERSION est défini dans Vercel, vérifier qu'il est au format YYYYMM.
const rawVersion = process.env.LINKEDIN_API_VERSION || "202501";
// Sanitize : si quelqu'un a mis "20250401" (8 chiffres), on tronque à 6
export const LINKEDIN_API_VERSION = /^\d{8}$/.test(rawVersion.trim())
  ? rawVersion.trim().slice(0, 6)
  : rawVersion.trim() || "202501";
...
export async function fetchCurrentLinkedInMember(accessToken) {
  // /v2/userinfo (OpenID Connect) — accessible avec les scopes openid + profile
  const profile = await fetchLinkedInJson("/v2/userinfo", accessToken);
  ...
  return {
    ...
    fullName: profile.name || [profile.given_name, profile.family_name].filter(Boolean).join(" ") || null,
  };
}
 ...
 * @param {string} ownerUrn  — e.g. "urn:li:person:xxx" or "urn:li:organization:xxx"
 * @param {Buffer} imageBuffer
 * @param {string} mimeType  — e.g. "image/jpeg"
 * @returns {Promise<string>}  — image URN like "urn:li:image:C5622AQExxxxxx"
 */
export async function uploadLinkedInImage(accessToken, ownerUrn, imageBuffer, mimeType = "image/jpeg") {
  // Step 1 — initialize upload
  const initRes = await fetch(`${LINKEDIN_API_BASE_URL}/rest/images?action=initializeUpload`, {
    method: "POST",
    headers: {
      ...getLinkedInHeaders(accessToken),
      "Content-Type": "application/json",
  ...
  if (!initRes.ok) {
    const msg = initData.message || initData.error || "LinkedIn image init failed.";
    throw new Error(msg);
  }
  ...
  if (!uploadUrl || !imageUrn) {
    throw new Error("LinkedIn image upload URL or URN missing in response.");
  }
