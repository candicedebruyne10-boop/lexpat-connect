              ...
              >
                ...
                {region.id === "flandre" && (
                  <div className="mt-5 flex flex-wrap gap-3">
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 text-[11px] font-semibold text-amber-700">
                      ✦ Dispense totale du test marché
                      <span className="font-normal text-amber-600">— AM 1 déc. 2025 · 21 professions</span>
                    </span>
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-[#d4e8e6] bg-[#f0faf9] px-3 py-1.5 text-[11px] font-semibold text-[#57b7af]">
                      ● Test marché 9 semaines (VDAB + EURES)
                    </span>
                  </div>
                )}
                ...
                <div className="mt-6 grid gap-5 xl:grid-cols-2">
                  {region.groups.map((group) => (
                    ...
                    >
                      <h3 className="text-lg font-semibold text-[#1E3A78]">
                        {translateGroupTitle(group.title, "fr")}
                      </h3>
                      <ul className="mt-4 space-y-2.5">
                        {group.jobs.map((job) => {
                          const label = translateProfessionLabel(job, "fr");
